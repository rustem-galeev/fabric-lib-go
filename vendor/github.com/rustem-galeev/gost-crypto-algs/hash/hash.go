...
const (
	// BlockSize 64 bytes or 512 bits
	BlockSize = 64
)
...
func (h *Hash) Write(data []byte) (int, error) {
	...
	for len(h.buf) >= BlockSize {
		...
		copy(h.chk, h.addMod512(h.chk, h.tmp))
		h.n += BlockSize * 8
...
func (h *Hash) Sum(in []byte) []byte {
	...
	copy(hsh, h.g(0, hsh, h.addMod512(h.chk, buf)))
	if h.size == 32 {
...
func (h *Hash) addMod512(first, second []byte) []byte {
	var ss uint16
	array := make([]byte, BlockSize)
	for i := 0; i < BlockSize; i++ {
		ss = uint16(first[i]) + uint16(second[i]) + (ss >> 8)
		array[i] = byte(0xFF & ss)
	}
	return array
}
...
func (h *Hash) g(n uint64, hsh, data []byte) []byte {
	array := make([]byte, BlockSize)
	copy(array, hsh)
	array[0] ^= byte((n >> 0) & 0xFF)
	array[1] ^= byte((n >> 8) & 0xFF)
	array[2] ^= byte((n >> 16) & 0xFF)
	array[3] ^= byte((n >> 24) & 0xFF)
	array[4] ^= byte((n >> 32) & 0xFF)
	array[5] ^= byte((n >> 40) & 0xFF)
	array[6] ^= byte((n >> 48) & 0xFF)
	array[7] ^= byte((n >> 56) & 0xFF)
	return common.Xor(
		h.buffer,
		common.Xor(
			h.buffer,
			h.e(l(array, h.ps(array)), data),
			hsh),
		data)
}
...
func (h *Hash) e(k, msg []byte) []byte {
	for i := 0; i < 12; i++ {
		msg = l(h.eMsgBuf, h.ps(common.Xor(h.buffer, k, msg)))
		k = l(h.eKBuf, h.ps(common.Xor(h.buffer, k, c[i][:])))
	}
	return common.Xor(h.buffer, k, msg)
}
